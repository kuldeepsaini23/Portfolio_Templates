...
const Work = () => {
  ...
  const [animateCard, setAnimateCard] = useState({y:0,opcatiy:1});
  ...
  function handleWorkFilter(item){
    ...
    setAnimateCard([{y:100, opcatiy:0}])

    setTimeout(()=>{
      setAnimateCard([{y:0, opcatiy:1}]);
      if(item === 'All'){
        setFilterWork(works);
      }else{
  ...
  return (
    <div>
      ...
      <div className='app__work-filter'>
        {['All','Web App', 'ReactJs', 'Mobile App'].map((item, index)=>(
